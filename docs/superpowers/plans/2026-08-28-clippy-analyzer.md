...
**A stopgap Task 4 left you to remove.** Task 4 changed `measure_repository`'s signature, which made `prune.py` and `report.py` call it wrongly — a mypy `call-arg` error each, and over an empty baseline that fails the gate at Task 4's own commit. So Task 4 passed `ANALYZER_NAMES` in both files: the same set the old one-argument signature measured, chosen to preserve behaviour exactly rather than to anticipate this task. **Both call sites, and both `ANALYZER_NAMES` imports, are yours to replace with `scope.to_measure`.** If `ANALYZER_NAMES` ends up unused in either module, drop it from the import or ruff's `F401` fails the gate.

**Two existing test files already carry a Python marker for the same reason.** Task 4 found that eight tests drive `run_check`/`run_freeze` against a directory holding nothing but `.ebpy/` artifacts, which now evidences no language and refuses before measuring. `tests/test_check.py`'s `_write_frozen_ceiling` writes a `pyproject.toml`, and `tests/test_freeze.py` gained `_evidence_python(cwd)`. Expect to need the same in `tests/test_prune.py`, and note that `tests/test_report.py` needs it **only** where a test wants a non-empty `to_measure` — an empty scope with a contract behind it is a case `report` must proceed through, and is worth a test of its own.
...
**One test Task 4 could not write, and now you can.** Task 4's plan text gave `check` a test freezing `("ruff", "clippy")` and asserting `"clippy" in result.message`. That test cannot pass before this task: with clippy unregistered, `(frozen ∩ registered) − detected` can never contain it — which is exactly the `no-runner` preservation `test_an_unregistered_frozen_analyzer_stays_a_no_runner_case` pins. Task 4 shipped the same property using `ruff` in a tree evidencing no Python (`test_check_refuses_before_measuring_when_the_contract_names_an_undetected_analyzer` in `tests/test_check.py`). Now that clippy is registered, add the clippy-named case beside it: a `("ruff", "clippy")` contract in a Python-only tree must refuse before measuring and name clippy. That is the first test in this repository where the two directions of rule 4 are distinguishable, so it is worth having.
...
- [ ] **Step 7: Regenerate `docs/shared-helpers.md`**

That file carries `<!-- Generated by \`ebpy catalog\`. Regenerate rather than editing. -->`, and it is stale from Task 4: it still lists `reconcile_scope` (deleted) and the old one-argument `measure_repository` docstring. Regenerate it with `ebpy catalog` rather than hand-editing, and check the diff only removes and updates what those tasks actually changed.

Nothing in CI checks this file, which is exactly why it needs a step — a stale generated catalogue is the kind of thing that survives a whole feature and misleads the next reader.

- [ ] **Step 8: Verify the repository still gates itself**
...
```bash
git add docs/measurement-seam.md docs/cli/ docs/shared-helpers.md CLAUDE.md src/ebpy/decide/__init__.py && git commit -m "docs: describe the analyzer scope, clippy, and the statuses report can hold"
```
